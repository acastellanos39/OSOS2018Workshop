---
title: "Accessing Data Repositories in R (GBIF and iNat)"
...
```
2) you can use `dplyr::filter`
```{r, eval = F}
filter(CNTY, NAME_1 == "Texas")
```
3) you can subset within square brackets
```{r, eval = F}
...
Soooo, you may be wondering what kind of mystical arts I'm attempting with `%$%` and `%>%`. I'm lazy, so I constructed a pipeline. Pipelines in R are created in using the handy `%>%` batch of symbols from the [magrittr](https://magrittr.tidyverse.org/) package. Essentially what it does is take the result from the left and use it as the first argument in whatever is after `%>%` (`%$%` is a special usage that only grabs a specific column or index from the left).
...
BRTC %>% filter(stateProvince == "Texas" & !is.na(decimalLongitude)) %>% st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = 4326) %>% st_set_crs(NA) %>% st_set_crs(st_crs(as(CNTY, "sf"))) %>% st_transform(crs = st_crs(as(CNTY, "sf"))) %>% st_intersection(as(CNTY, "sf")) %>% mapview
```
...
But not all species richness is created the same. Five *Peromyscus* species are different than a community that has marsupials, rodents, and cervids, for example. But how do we show this...
